...
# @click.argument("name")
@click.option("-i", "--install", help="input the name of the program", nargs=1)
@click.option("-s", "--search", help="input the name of the program", nargs=1)
def main(install=None, search=None):
    count = 0
    lists_count = 0
...
count += 1
# if get_name == install:
# anzhuang(get_name, get_url, get_parameter, get_fs)
f.close()
...
@click.option("-i", "--install", help="input the name of the program", nargs=1)
@click.option("-s", "--search", help="input the name of the program", nargs=1)
@click.option("-g", "--filesize", help="None", nargs=1)
def main(install=None, search=None,filesize=None):
    count = 0
    lists_count = 0
    for i in range(line_count):
        get_name = names[count]
    ...
    if search != None:
        ...
        lists_count += 1
    if filter != None:
        if fnmatch.fnmatch(names[count], filesize) == True:
